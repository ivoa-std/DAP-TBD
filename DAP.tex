...
\previousversion{This is the first published version}
...
SIA defines data discovery and metadata capabilities that work with other DAL services to enable image and data cube access. The basic interface for the capabilities defined in this specification are described in DALI \citep{2017ivoa.spec.0517D}. DataLink can be used with SIA for finding access URL(s) for files, related resources, and data services such as AccessData (in development). SIA services also support VOSI-availability\todo{Please don't talk about availability; it ought to be phased out in its current form} and VOSI-capabilities \citep{2017ivoa.spec.0524G}  resources.
...
SIA specifies standardID values for each capability, as defined by VODataService \citep{2021ivoa.spec.1102D}. SIA services may be registered in an IVOA Registry using the SimpleDALRegExt \citep{2017ivoa.spec.0530P}\todo{Since that doesn't say anything about DAP, we shouldn't cite it here but come up with a registry extension ourselves (if neccessary)} extension schema.
...
Values used in the TIME parameter are always interpreted as specified in DALI in the section on literal values: UTC time scale and UNKNOWN reference position \citep{2007ivoa.spec.1030R}\todo{Please cite the refpos vocabulary here instead}. Modified Julian Date values are always in days.
...
Note: Spaces in parameter values must be URL-encoded as \%2B or +; we leave this out of the example to make it easier to read\todo{Let's not talk about URL quoting here; it's complicated (e.g., no + in POST payloads}.\\
...
\end{lstlisting}
         How do I query a service containing radio cubes from Alma operated by NRAO in a circle of 0.1 deg around position 180.47 -18.70 in the CO band in the range 800 microns to 900 microns and made in the time range between Mjd=55708 and 55710?\todo{Let's not include the full responses in the document: It's a bad waste of space.  Rather, make them separate files, validate them in a make test, and link to them using auxiliaryurl.  Don't forget to include their names in AUX\_FILES.} \\
...
Fix typos.\todo{Let's remove the SIA history here.  DAP is a new standard and has its own history.  The SIA history can go to acknowledgements or the introduction, I guess.}
